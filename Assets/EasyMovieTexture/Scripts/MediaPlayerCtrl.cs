...
public class MediaPlayerCtrl : MonoBehaviour
{
	...
	private void Call_UpdateVideoTexture()
	{
		...
		if( fLastFrameTime > fCurrentSeekTime - 0.1f  )
		{
            ...
            {
				//Debug.Log ("list audiocount.....");
				//Debug.Log ("list audiocount.1...."+(float)listAudioPtsTime[i]);
				if(listAudioPtsTime[i] != null)
...
public void LoadUrlDownload (string url, Action<bool> callback)
{
	StartCoroutine (DownloadStreamingVideoAndLoad2 (url, callback));
}

public void LoadUrlCopy (string url)
{
	StartCoroutine (CopyStreamingAssetVideoAndLoad (url));
}

public IEnumerator DownloadStreamingVideoAndLoad2(string strURL, Action<bool> callback)
    {
        strURL = strURL.Trim();

		string filename = ConvertLongString (strURL.Substring (strURL.LastIndexOf ("/")));
		string write_path = Application.persistentDataPath + "/Data/" + filename;
		Debug.Log (filename);
